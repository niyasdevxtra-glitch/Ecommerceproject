...
import './AnimatedAddToCart.css';

export default function AnimatedAddToCart({ onClick, onComplete, className = '', children = 'Add to cart' }) {
    ...
    useEffect(() => {
        ...
        const ctx = gsap.context(() => {
            // Try to register MorphSVGPlugin if available on window (e.g. CodePen CDN script)
            ...
            const handleClick = (e) => {
                ...
                // Only try morphing if MorphSVGPlugin exists, otherwise skip to prevent plugin warnings
    ...
    return (
        <button ref={buttonRef} className={`add-to-cart flex-1 w-full ${isActive ? 'active' : ''} ${className}`}>
            <span>{children}</span>
